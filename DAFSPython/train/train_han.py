...
# 计算 Hits@N 的函数
def calculate_hits_at_n(model, test_dataloader, device=None):
    model.eval()
    k_values = [1, 5, 10]
    ...
    with torch.no_grad():
        for text_vec, image_vec, label in test_dataloader:
            ...
            # 计算 Hits@K
            for k in k_values:
                hits_at_k[k] += torch.sum((top_k_idx[:, :k] == label.view(-1, 1).to(torch.int64)).float()).item()
...
for epoch in range(100):
    ...
    if epoch % 10 == 0:
        ...
        # 计算 Hits@K
        hits_at_k = calculate_hits_at_n(model, test_dataloader, device=device)
